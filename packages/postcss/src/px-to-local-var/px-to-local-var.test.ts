...
describe('postcssPxToRem 插件', () => {
  ...
  it('支持自定义 varName', async () => {
    ...
    const output = await postcss([postcssPxToLocalVar({ varName: '--custom-rem' })]).process(input, {
      from: undefined,
    });
  ...
  it('replace: false 时应保留原始 px', async () => {
    ...
    const output = await postcss([postcssPxToLocalVar({ replace: false })]).process(input, { from: undefined });
  ...
  it('小于等于 minPixelValue 的 px 不应被转换', async () => {
    ...
    const output = await postcss([postcssPxToLocalVar({ minPixelValue: 2 })]).process(input, { from: undefined });
    expect(output.css).toBe(input);
  });

  it('只转换 propList 指定的属性', async () => {
    const input = `.a { font-size: 24px; margin: 24px; }`;
    const output = await postcss([postcssPxToLocalVar({ propList: ['font-size'] })]).process(input, {
      from: undefined,
    });
  ...
  it('selectorBlackList 指定的选择器不应被转换', async () => {
    ...
    const output = await postcss([postcssPxToLocalVar({ selectorBlackList: ['.ignore'] })]).process(input, {
      from: undefined,
    });
  ...
  it('exclude 配置命中的文件不应被转换', async () => {
    ...
    const output = await postcss([postcssPxToLocalVar({ exclude: /test\.css$/ })]).process(input, { from: 'test.css' });
    expect(output.css).toBe(input);
  });

  it('mediaQuery: true 时应转换媒体查询中的 px', async () => {
    const input = `@media (min-width: 320px) { .a { margin: 16px; } }`;
    const output = await postcss([postcssPxToLocalVar({ mediaQuery: true })]).process(input, { from: undefined });
    expect(output.css).toBe(
  ...
  it('mediaQuery: false 不转换媒体查询中的 px', async () => {
    const input = `@media (min-width: 320px) { .a { margin: 16px; } }`;
    const output = await postcss([postcssPxToLocalVar({ mediaQuery: false })]).process(input, { from: undefined });
    expect(output.css).toBe(`@media (min-width: 320px) { .a { margin: calc(var(--local-scope-rem, 1rem) * 1); } }`);
  });

  it('遇到 /* pxtovar-disable-next-line */ 注释应跳过转换', async () => {
    const input = `/* pxtovar-disable-next-line */\n.a { margin: 16px; }\n.b { margin: 16px; }`;
    const output = await postcss([postcssPxToLocalVar()]).process(input, { from: undefined });
    expect(output.css).toBe(
